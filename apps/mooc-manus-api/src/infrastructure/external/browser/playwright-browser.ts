...
export class PlaywrightBrowser implements BrowserInterface {
  ...
  constructor(cdpUrl: string, llm: LlmClient | null = null, logger: Logger) {
    this.logger = logger;
    this.cdpUrl = cdpUrl;
    this.llm = llm;
  }
  ...
  async initialize(): Promise<boolean> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        this.logger.warn(
          `Failed to initialize Playwright Browser, retrying in ${i + 1} times, ${retryInterval} seconds..., error: {error}`,
          { error },
        );
  ...
  async cleanup() {
    try {
      ...
      this.logger.error('Failed to cleanup Playwright Browser', { error });
    } finally {
      this.interactiveElementsCache = [];
      this.page = null;
      this.browser = null;
