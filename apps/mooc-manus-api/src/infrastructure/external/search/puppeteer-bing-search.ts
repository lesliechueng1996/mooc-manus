...
export class PuppeteerBingSearch implements SearchEngine {
  ...
  async search(
    ...
    try {
      ...
      try {
        this.logger.info('Waiting for search results selector...');
      ...
      this.logger.info(`Page loaded, HTML length: ${html.length}`);
      ...
      if (html.length < 5000) {
        ...
        this.logger.info(`HTML preview: ${html.substring(0, 500)}`);
      }
      ...
      this.logger.info(`Found ${resultElements.length} result elements`);
      ...
      for (const resultElement of resultElements) {
        try {
          ...
          this.logger.warn(`Failed to parse Bing search result item`, {
            error,
          });
        }
      }
      ...
      return {
        success: true,
        message: null,
        data: searchResultsSchema.parse({
          query,
          dataRange,
          totalResults,
      ...
      this.logger.error(`Failed to search Bing with Puppeteer`, { error });
      const errorResult = searchResultsSchema.parse({
        query,
        dataRange,
        totalResults: 0,
        results: [],
